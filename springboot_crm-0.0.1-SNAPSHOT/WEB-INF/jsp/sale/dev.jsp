...
<!-- 替换条件查询 -->	
<form action="../sale/devList.action" id="salChance">
	...
	<c:forEach items="${pageInfo.lists}" var="saleChance" >
		<tr>
			...
			<c:if test="${saleChance.chcStatus==2}">
				<img onclick="to('../plan/toDevPlan.action?chcId=${saleChance.chcId}');" title="制定计划" src="../images/bt_plan.gif" class="op_button" />
				<img onclick="to('../plan/toDevExecute.action?chcId=${saleChance.chcId}');" title="执行计划" src="../images/bt_feedback.gif" class="op_button" />
				<img onclick="alert('用户开发成功，已添加新客户记录。');window.location.href='../plan/result.action?chcId=${salChance.chcId}&chcStatus=3';" title="开发成功" src="../images/bt_yes.gif" class="op_button" />
			</c:if>
			<c:if test="${saleChance.chcStatus==3||saleChance.chcStatus==4}">
				
				<img onclick="to('../plan/toDevDetail.action?chcId=${saleChance.chcId}');" title="查看" src="../images/bt_detail.gif" class="op_button">
			</c:if>
			</td>
...
<form action="../sale/devList.action" id="go">
<div class="pager">
	...
	<c:choose> 
	     ...
	     <c:otherwise>  <!--否则 -->    
	       <a href="../sale/devList.action?pageNum=1&chcCustName=${pageInfo.entity.chcCustName}&chcTitle=${pageInfo.entity.chcTitle}&chcLinkman=${pageInfo.entity.chcLinkman}&pageSize=${pageInfo.pageSize}">首页</a>
	      </c:otherwise> 
    </c:choose>
	...
	<c:choose> 
	     ...
	     <c:otherwise>  <!--否则 -->    
	       <a href="../sale/devList.action?pageNum=${pageInfo.pageNum-1}&chcCustName=${pageInfo.entity.chcCustName}&chcTitle=${pageInfo.entity.chcTitle}&chcLinkman=${pageInfo.entity.chcLinkman}&pageSize=${pageInfo.pageSize}">上一页</a>
	      </c:otherwise> 
    </c:choose>
	...
	<c:choose> 
	     ...
	     <c:otherwise>  <!--否则 -->    
	       <a href="../sale/devList.action?pageNum=${pageInfo.pageNum+1}&chcCustName=${pageInfo.entity.chcCustName}&chcTitle=${pageInfo.entity.chcTitle}&chcLinkman=${pageInfo.entity.chcLinkman}&pageSize=${pageInfo.pageSize}">下一页</a>
	      </c:otherwise> 
    </c:choose>
	...
	<c:choose> 
	     ...
	     <c:otherwise>  <!--否则 -->    
	       <a href="../sale/devList.action?pageNum=${pageInfo.totalPage}&chcCustName=${pageInfo.entity.chcCustName}&chcTitle=${pageInfo.entity.chcTitle}&chcLinkman=${pageInfo.entity.chcLinkman}&pageSize=${pageInfo.pageSize}">最后一页</a>
	      </c:otherwise> 
   </c:choose>
